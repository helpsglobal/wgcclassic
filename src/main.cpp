...
#if defined(NDEBUG)
# error "Wgcclassic cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x536e90369c107486b7bf64ce7ee740d68ff19105882850cfab7742b8705a2dd1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wgcclassic: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wgcclassic Signed Message:\n";
...
{
    // Wgcclassic: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Wgcclassic: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Wgcclassic: 2 minutes
static const int64 nTargetSpacing = 1 * 30; // Wgcclassic: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wgcclassic: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wgcclassic: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x536e90369c107486b7bf64ce7ee740d68ff19105882850cfab7742b8705a2dd1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "this is my fist coin to start with";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x6a8d177ca529da65b6be6fe4a3c6f578ca6e8452dc96de8f2c73821670795e85"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Wgcclassic: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WgcclassicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WgcclassicMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WgcclassicMiner(CWallet *pwallet)
{
    printf("WgcclassicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wgcclassic-miner");
    ...
    try { loop {
        ...
        printf("Running WgcclassicMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WgcclassicMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WgcclassicMiner, pwallet));
}
